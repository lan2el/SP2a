...
float dist_prev = _DIST_MAX;        // Distance last-measured (initial value)
...
void loop() {
  ...
  if ((dist_raw == 0.0) || (dist_raw > _DIST_MAX)) {
      dist_raw = dist_prev;    // Set Higher Value 이 아니라 이전에 측정했던 값으로 
      digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (dist_raw < _DIST_MIN) {
      dist_raw = dist_prev;    // Set Lower Value 얘도 이전값을 사용하도록 
      digitalWrite(PIN_LED, 1);       // LED OFF
  ...
  // Set previous ema value (dist_prev)
